...
class KMP:
    ...
    # a b a j a-a*b a j j*a b a j a b
    # a b a j a-b*                       j = 0, 1, 2, 3, 4, 5. i = 0, 1, 2, 3, 4, 5 -> T[5] != P[5]
    #         a-b*                       j = f(4) = 1. i = 5  -> T[5] != P[1], check prefix = suffix to skip
    #           a b a j a*               j = f(1) = 0, 1, 2, 3, 4. i = 5, 6, 7, 8, 9 -> T[9] != P[4]
    #                   a*               j = f(3) = 0. i = 9 -> T[9] != P[0]
    #                     a b a j a b    j = 0, 1, 2, 3, 4, 5. i = 10, 11, 12, 13, 14, 15, now T[14] = P[5], end
    ...
    def failure_function(self, pattern: MyString) -> list:
        ...
        # b a j a b         i=1, j=0, f(1) = 0
        # a*

        # b a j a b         i=2, j=0, f(2) = 1
        #   a

        # b a j a b         i=3, j=1, T[3] != P[1]
        #   a b*

        # b a j a b         i=3, j=0, T[3] != P[0], f(3) = 0
        #     a*

        # b a j a b         i=4, j=0, T[4] = P[0], f(4) = 1
        #       a

        # b a j a b         i=5, j=1, T[5] = P[1], f(5) = 2
        #       a b
...
string = MyString(['a', 'b', 'a', 'j', 'a', 'a', 'b', 'a', 'j', 'j', 'a', 'b', 'a', 'j', 'a', 'b'])
pattern1 = MyString(['a', 'b', 'a', 'j', 'a', 'b'])
pattern2 = MyString(['j', 'a'])
pattern3 = MyString(['a', 'j', 'j'])
pattern4 = MyString(['d'])
pattern5 = MyString(['a', 'b', 'a', 'b', 'j'])
